...
class FFM(nn.Module):
    ...
    def forward(self, a, delta_a, b, delta_b, history, delta_history):
        a = to_tensor_batched(a, 1).unsqueeze(1)
        b = to_tensor_batched(b, 1).unsqueeze(1)
...
class Early(nn.Module):
    ...
    def forward(self, a, delta_a, b, delta_b, history, delta_history):
        a = to_tensor_batched(a, 1)
        b = to_tensor_batched(b, 1)
        delta_a = to_tensor_batched(delta_a, 1)
        delta_b = to_tensor_batched(delta_b, 1)
        history = to_tensor_batched(history, 2)
...
def get_samples(
    ...
    while len(samples) < n_samples:
        ...
        a_first_occ = bisect.bisect_left(occurences[a_mapped], t) or float("inf")
        b_first_occ = bisect.bisect_left(occurences[b_mapped], t) or float("inf")
...
def train_model(
    ...
    return loss_a_b_sum, loss_delta_sum
...
def test_on_sequence(
    ...
    def get_to_evict(t) -> int:
        ...
        distr = solve_linear_program(linear_program_input, cache_tuple)

        if greedy:
            return cache_tuple[np.argmax(distr)]
        else:
            return random.choices(cache_tuple, weights=distr)[0]
    ...
    for t, addr in pbar:
        ...
        if t % train_interval == 0 and t >= train_interval * 2:
            ...
            loss_a_b, loss_delta = train_model(
                model,
                optimizer,
                samples,
